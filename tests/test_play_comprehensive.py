...
def test_play_comprehensive(auth_page: Page):
    """
    Comprehensive test to create a play and then delete it.
    """
    # 1. Start from dashboard
    auth_page.goto("https://admin.cryptocat.ssd.uz/admin")
    
    # 2. Navigate to Plays tab
    print("\n[STEP 1] Navigating to Plays...")
    plays_link = auth_page.locator("a.fi-sidebar-item-button").filter(has_text="Plays")
    plays_link.click()
    auth_page.wait_for_url("**/admin/plays**")
    
    # 3. Click New play button
    print("[STEP 2] Creating a new play...")
    create_button = auth_page.locator("button, a").filter(has_text="New play").first
    create_button.click()
    
    # 4. Fill in the form
    timestamp = int(time.time())
    test_title = f"Test Play {timestamp}"
    test_slug = f"test-play-{timestamp}"
    test_body = f"This is a test play body created at {timestamp}."
    ...
    # Slug/Link field
    auth_page.locator('#mountedActionsData\\.0\\.link').fill(test_slug)
    print(f"[OK] Filled slug: {test_slug}")
    
    # Cover image upload
    temp_dir = tempfile.gettempdir()
    test_image_path = os.path.join(temp_dir, f"test_play_{timestamp}.png")
    ...
    print(f"[OK] Uploaded cover image")
    ...
    # We'll retry once if we're still on the create page
    auth_page.wait_for_timeout(2000)
    if auth_page.locator(".fi-modal-header").filter(has_text="Create play").is_visible():
        print("[WARNING] Creation modal still visible, possibly due to validation error. Retrying submit...")
        # Check if there are visible errors
        errors = auth_page.locator(".fi-fo-field-wrp-error-message").all_inner_texts()
        if errors:
            print(f"[INFO] Found errors: {errors}")
            # Try to re-click editors to trigger sync
            for editor in body_editors:
                editor.click()
    ...
    print("\n[STEP 3] Verifying play creation...")
    search_input = auth_page.locator("input[placeholder*='Search']").first
    search_input.fill(test_title)
    search_input.press("Enter")
    auth_page.wait_for_timeout(2000)
    
    play_row = auth_page.locator("table.fi-ta-table tr").filter(has_text=test_title).first
    expect(play_row).to_be_visible()
    print(f"[OK] Play '{test_title}' found in the list.")
    
    # 7. Delete the play
    print(f"\n[STEP 4] Deleting the play '{test_title}'...")
    delete_btn = play_row.locator("button, a").filter(has_text="Delete").first
    ...
    empty_state = auth_page.get_by_text("No plays", exact=False).or_(auth_page.get_by_text("No results", exact=False))
    ...
    print(f"[SUCCESS] Play '{test_title}' is definitively gone.")
